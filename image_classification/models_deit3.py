...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,init_values=1e-6,
...
class ResFormer(nn.Module):
    ...
    def __init__(self, img_size=(224,), patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embed = embed_layer(img_size=img_size[0], patch_size=patch_size, in_chans=in_chans,
                                       embed_dim=embed_dim)
    ...
    def forward_features(self, x, distillation_target='logit'):
        ...
        if distillation_target == 'gap':
            return self.pre_logits(x[:, 0]), F.adaptive_avg_pool1d(x[:, self.num_tokens:].transpose(1, 2),
                                                                   (1,)).flatten(1).squeeze(-1)
        else:
...
@register_model
def resformer_base_patch16_deit3b_eval(pretrained=False, **kwargs):
    model = ResFormer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    # model.default_cfg = _cfg()
...
@register_model
def resformer_base_patch16_vitb_21k_eval(pretrained=False, **kwargs):
    model = ResFormer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    # model.default_cfg = _cfg()
